...
function _devenv_autocomplete {
    ...
    _devenv_commands="--ctags-exclude= --daemonize --help --target= --tmux --tmux= --tmux-prefix= --path= attach build-tags init help kill ls pause refresh restart reload rename self-update start stop unpause"
    COMPREPLY=()
