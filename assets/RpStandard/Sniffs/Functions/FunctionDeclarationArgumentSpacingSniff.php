 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class RpStandard_Sniffs_Functions_FunctionDeclarationArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
    /**
        ...
        while (($nextParam = $phpcsFile->findNext(T_VARIABLE, ($nextParam + 1), $closeBracket)) !== false) {
            ...
            if ($nextComma !== false) {
                ...
                if ($tokens[($nextComma - 1)]['code'] === T_WHITESPACE) {
                    if ($tokens[($nextComma - 1)]['content'] !== $phpcsFile->eolChar && $tokens[($nextComma - 2)]['content'] !== $phpcsFile->eolChar) {
