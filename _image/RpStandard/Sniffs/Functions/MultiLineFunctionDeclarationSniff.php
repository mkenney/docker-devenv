 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class RpStandard_Sniffs_Functions_MultiLineFunctionDeclarationSniff extends PEAR_Sniffs_Functions_FunctionDeclarationSniff
{
    ...
    public function processBracket(PHP_CodeSniffer_File $phpcsFile, $openBracket, $tokens, $type='function')
    {
        ...
        for ($i = ($openBracket + 1); $i < $closeBracket; $i++) {
            ...
            $var_count = 0;
            $token = $i;
            while ($tokens[$token]['content'] !== $phpcsFile->eolChar) {
                if ($tokens[$token]['type'] === 'T_VARIABLE') {
                    $var_count++;
                }
                if ($var_count > 1) {
                    $error = 'Multi-line '.$type.' declarations must define one parameter per line';
                    $fix = $phpcsFile->addFixableError($error, $token, $errorPrefix.'OneParamPerLine');
                    if ($fix === true) {
                        $phpcsFile->fixer->addNewline($token);
                    }
                    break;
                }
                $token++;
